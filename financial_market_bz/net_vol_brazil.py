######################################################################
# script to break down Brazil volatility
...
dv = pd.read_csv("../data/stock_vol_imp.csv", index_col=0,
                 usecols=[0, 1, 3], header=0,
                 na_values=".").dropna()
dv.columns = ["us", "bz"]

results = smf.ols(formula='bz ~ us', data=dv).fit()
...
fig = gen_chart(df, "Brazilian Net Volatility", "%", "2012-01-01")
py.image.save_as(fig, "../exhibits/brazilian_vol.jpeg", format="jpeg")
