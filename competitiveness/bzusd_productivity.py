...
df = pd.read_csv('../data/cut.csv', index_col=0, usecols=[0,1])
...
## chart
trace01 = go.Scatter(x=df_final.index, y=df_final.iloc[:,0], name = "Labor Productivity")
trace02 = go.Scatter(x=df_final.index, y=df_final['average'], name = "Average",
                     line=dict(color="red", dash="dot"))
data = [trace01, trace02]
layout = go.Layout(title="<b> Deflated Labor Productivity in USD - Brazil </b>",
                   font=dict(size=18), legend=dict(x=0, y = -0.2),
                   yaxis=dict(title="pts", tickmode="auto", nticks=5))
fig = go.Figure(data=data, layout=layout)
py.image.save_as(fig, '../exhibits/usdbr_productivity.jpeg',
                 format="jpeg")
